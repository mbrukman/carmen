...
<<<<<<< HEAD
            ...
            phrase = termops.removeWhiteSpace(phrase);
...
=======
            ...
            var tryNormalization = source.use_normalization_cache && (!subquery.ender || text.length >= 3);
            var dawgMatch = tryNormalization ?
                dawg.hasPhraseOrNormalizations(text, subquery.ender) :
                dawg.hasPhrase(text, subquery.ender);

            if (!dawgMatch) continue;

            var variants;
            if (!tryNormalization) {
                variants = [{text: subquery.join(' '), ender: subquery.ender}];
            } else if (subquery.ender) {
                variants = [{text: subquery.join(' '), ender: subquery.ender}];
                for (let v of dawgMatch.normalizations) {
                    variants.push({text: v, ender: false});
                }
>>>>>>> master
            } else {
                variants = dawgMatch.normalizations.length ? [{text: dawgMatch.normalizations[0], ender: subquery.ender}] : [{text: subquery.join(' '), ender: subquery.ender}];
            }
            ...
            for (let variant of variants) {
                ...
                var prefix = (options.autocomplete && variant.ender && !(source.geocoder_address && termops.isAddressNumber(text)));
