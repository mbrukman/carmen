...
// removeWhiteSpace - collapses a query for fuzzy search
/**
* @param {String} - a single query
* @return {String} - a query without spaces
*/
function removeWhiteSpace(query) {
  var noSpace = query.split(' ');
...
function getIndexableText(replacer, globalTokens, doc) {
    ...
    // if this doc has translations, we only index the default text for the 'default' language
    // so it would get penalized as a not-in-language match if someone were doing a language-specific search,
    // where we'd want the language-specific text to win
    //
    // alternatively, if this doc has no translations, there are no language-specific texts that could win
    // so this feature would always get penalized in a language-specific search, which we don't want, so
    // index it as all-language text

    if ((!langTexts.size) || (langTexts.size === 1) && (doc.properties['carmen:text_universal'])) {
        getTokens(texts, 'all');
    } else {
...
function decollide(replacer, doc, subq) {
    subq = removeDiacritics(subq.toLowerCase()).toLowerCase().trim();
    ...
    for (var i = 0; i < length; i ++) {
        var code = keys[i].match(/text_(.+)/);
        if (code && cl.hasLanguage(code[1]) && doc.properties[keys[i]]) {
            texts = texts.concat(token.replaceToken(replacer, doc.properties[keys[i]]).split(','));
        }
    }
    ...
    var fails = 0;
    while (a--) {
        var text = removeDiacritics(texts[a].toLowerCase()).toLowerCase().trim();
        var textHash = {
            32: true, // ' ' for spaces
        };
        var b = text.length;
