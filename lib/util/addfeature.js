...
function _addFeature(source, docs, vtile_only, callback) {
    ...
    docs.forEach((input) => {
        ...
        if (options.geocoder) {
            for (let token of termops.tokenize(input.properties['carmen:text'])) {
                options.geocoder.dic.add(token);
            }
        }
...
function vectorize(feat, source, done) {
    ...
    let zxys = cover.tiles(feat.geometry, {min_zoom: source.maxzoom, max_zoom: source.maxzoom}).map((xyz) => {
        let x = xyz[0];
        let y = xyz[1];
        let z = xyz[2];
    ...
    for (let i = 0; i < zxys.length; i++) q.defer((zxy, done) => {
        source.getTile(zxy[0],zxy[1],zxy[2], (err, res) => {
            if (err) {
                ...
                let tmpTile = new mapnik.VectorTile(zxy[0],zxy[1],zxy[2]);
