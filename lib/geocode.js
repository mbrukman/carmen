...
module.exports = function(geocoder, query, options, callback) {
    ...
    if (options.language) {
        ...
        var invalidLanguages = options.language.filter(function(language) {
...
function idGeocode(geocoder, asId, options, callback) {
    ...
    q.awaitAll(function(err, features) {
        ...
        for (var i = 0; i < features.length; i++) {
            ...
            f.relevance = 1;
            resultIndexes[features[i].properties['carmen:index']] = true;
...
function forwardGeocode(geocoder, query, options, callback) {
    ...
    let suggest = ''
    let isSuggest = false;
    for (let token of queryData.query) {
        let suggestion = geocoder.dic.suggest(token);

        if (suggestion.length) isSuggest = true;

        suggest = `${suggest} ${suggestion[0] ? suggestion[0] : token}`;
    }
    if (isSuggest) queryData.suggestions.push(suggest.trim());
