...
function loadDoc(freq, patch, doc, source, zoom, token_replacer, globalTokens) {
    ...
    var texts = termops.getIndexableText(token_replacer, globalTokens, doc, source ? source.use_normalization_cache : false);
    var allPhrases = new Map();
    var phrase, phraseObj, languages;
    var variants = new Map();
    for (var i = 0; i < texts.length; i++) {
        var phrases = termops.getIndexablePhrases(texts[i].tokens, freq, source);
        for (var j = 0; j < phrases.length; j++) {
            phrase = phrases[j].phrase;

            if (!allPhrases.has(phrase)) allPhrases.set(phrase, {languages: new Set(), phrase: phrases[j]});
            ...
            if (phrases[j].relev > phraseObj.phrase.relev) phraseObj.phrase.relev = phrases[j].relev;

            texts[i].languages.forEach(function(l) { phraseObj.languages.add(l); });

            if (DEBUG && !phrases[j].degen) {
                console.warn('[%d] phrase: %s @ %d', doc.id, phrases[j].text, phrases[j].relev);
            }
        }
        if (texts[i].variants) {
            for (var k = 0; k < texts[i].variants.length; k++) {
                variants.set(termops.encodableText(texts[i].variants[k].join(' ')), termops.encodableText(texts[i].tokens.join(' ')));
            }
        }
    }
